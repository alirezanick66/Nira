...
- [x] **Async Logging**: زیرساخت PostgreSQL برای `query_logs` با مدیریت مستقل Session
- [x] **ماژولارشدگی فرانت‌اند**: تفکیک ساختاریافته `css/` و `js/` + ES Modules نیتیو
- [x] **دروازه امنیتی API**: میان‌افزار `ApiKeyMiddleware` با پشتیبانی همزمان از Header و Query Parameter
...
|    🏷️ **نرمال‌سازی هوشمند برندها**    |                           مپ کردن خودکار مترادف‌ها و تایپوها به نام کاننیکال                           | ✅ تکمیل |         TokenParser + YAML Config         |               بازگشت نتایج صحیح حتی با نام‌های غیررسمی                |
| **🧠 تشخیص نیت هیبریدی (Hybrid NLU)**  |    ترکیب Rule-Based (سریع) و Semantic Embedding (دقیق) برای پوشش کامل جملات محاوره‌ای و استعاره‌ای     | ✅ تکمیل |      NLUPipeline + Embedding Service      |                  دقت >۹۵٪ در تشخیص Intentهای پیچیده                   |
...
|      **🔄 Refine + Memory**      | پشتیبانی کامل از `Intent: refine` با استفاده از تاریخچه مکالمه | ✅ تکمیل  | `LLMOrchestrator` + `ConversationMemory`  |    درک صحیح مرجع مقایسه در ۹۵٪ کوئری‌های refine    |
...
| نگهداری موازی دو Endpoint (REST/SSE) | 🟡 متوسط | 🟡 متوسط | ✅ پیاده‌سازی `SearchService` مشترک. لاجیک تکراری حذف شد و هر دو Endpoint فقط آداپتور خروجی هستند. |
