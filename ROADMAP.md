...
📍 **وضعیت فعلی پروژه:** ورود رسمی به فاز `MVP Refinement` (تمرکز بر پارسینگ اعداد محاوره‌ای، کالیبراسیون Reranker و Smart Fallback)
...
| **💬 Clarification Loop**      | پرسش هدایت‌گر هوشمند وقتی کوئری کاربر مبهم است                 | 🔴 بالا  | NLU Confidence Score + Prompt Engineering | کاهش ۳۰٪ کوئری‌های `0 نتیجه` + افزایش رضایت کاربر           |
...
| **🔄 Refine + Memory**         | پشتیبانی کامل از `Intent: refine` با استفاده از تاریخچه مکالمه | 🔴 بالا  | `ConversationMemory` + Context Injection  | درک صحیح مرجع مقایسه در ۹۵٪ کوئری‌های refine                |
| **👍 بازخورد Like/Dislike**    | ثبت بازخورد کاربر + یادگیری سبک سلیقه (بدون Fine-tuning)       | 🟡 متوسط | Feedback Endpoint + Aggregator سبک        | امکان به‌روزرسانی `domain_knowledge.json` بر اساس بازخوردها |
...
| اشتباه گرفتن واحدها (GB/MB)            | 🟡 پایین | 🔴 بالا (تجربه کاربری) | تفکیک صریح `unit` در Payload + اعتبارسنجی در `ProductEnrichment`         |
