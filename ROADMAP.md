...
| :------------------------------------: | :----------------------------------------------------------------------------------------------------: | :-----: | :---------------------------------------: | :-------------------------------------------------------------------: |
|   **🔢 پارسینگ پیشرفته اعداد/واحد**    | پشتیبانی از «سی میلیون»، «30تومن»، «حدود ۴۰-۵۰» + فیلتر کردن اعداد مدل (`15`, `S24`) بدون Regex شکننده | ✅ تکمیل |  `NLUPipeline` + `domain_knowledge.json`  |           کاهش ۹۰٪ خطای استخراج عدد از کوئری‌های محاوره‌ای            |
...
|       **🧩 مدیریت تضاد فیلترها**       |               تشخیص کوئری‌های متناقض («ارزون ولی پرچم‌دار») → هشدار یا اولویت‌بندی پویا                | ✅ تکمیل |    `NLUPipeline` + `LLM Orchestrator`     |        جلوگیری از بازگشت نتایج نامربوط به‌خاطر فیلترهای متناقض        |
...
| پیچیدگی نگهداری `domain_knowledge.json` در چنددامنه | 🔵 پایین | 🟡 متوسط |          مهاجرت تدریجی به `TokenMatcher` + لایه `DomainRouter`           |
